...
def makesGroverCircuit_manual(n, oracle=None, dirty=False):
    nbIter = 1
...
def generate_grover(n, marked_item=0, dirty=True, output=None):
    """Generate one-round Grover circuit and export to QASM2 file."""
    ...
    if output is None:
        ...
        output = f"grover_n{n}_{anc_type}_{marked_item}_r1.qasm"
...
def main():
    parser = argparse.ArgumentParser(description="Generate one-round Grover quantum search circuit")
    parser.add_argument("n", type=int, nargs="?", help="Number of qubits")
    ...
    parser.add_argument(
        "--batch",
        action="store_true",
        help="Generate the repository's standard one-round Grover benchmark set",
    )
    ...
    if args.batch:
        for n in [5, 10, 20, 30, 40, 50, 70, 90, 100, 120, 140, 160, 180, 200, 240, 280, 300, 350, 400, 450, 500, 600, 700]:
            generate_grover(n, marked_item=args.marked_item, dirty=dirty, output=f"grover_dirty_n{n}_r1.qasm")
        return

    if args.n is None:
        parser.error("n is required unless --batch is used")
