...
def create_verified_identity(num_qubits, gate_count, allowed_gates, seed=None):
    output_name = f"identity_q{num_qubits}_g{gate_count}_s{seed}.qasm"

    U = generate_robust_random_u(num_qubits, gate_count, allowed_gates, seed=seed)

    pm = generate_preset_pass_manager(optimization_level=3, basis_gates=allowed_gates, seed_transpiler=seed)
    U_opt_inverse = pm.run(U.inverse())

    full_qc = U.compose(U_opt_inverse)
...
if __name__ == "__main__":
    ...
    for k in range(1):
        for n in range(50, 151, 50):
            create_verified_identity(num_qubits=n, gate_count=5 * n, allowed_gates=gates, seed=k)